...
1. `resume-blocked-high-value`: a recent ticket/PR/run has an agent-solvable
   blocker and clear retry evidence.
2. `finish-visible-incomplete`: a manager-visible workflow is partly built but
   lacks validation, entry point, rollback, or follow-up registration.
3. `adopt-positive-pilot`: a useful experiment has evidence and needs default
   adoption, scale issue, watch trigger, or rejection with evidence.
4. `refresh-evidence`: a promising external or backlog candidate is stale,
   list-only, or lacks primary-source validation.
5. `new-exploration`: no higher-priority evidence-backed work is available.

For every top recommendation, report the rank label, evidence level, manager
burden, and next executable action. Do not recommend a shiny external repo over
an unfinished manager-visible blocker unless the blocker is manager-only or hard
external.

## Evidence Contract

Static scans and GitHub list output can prioritize work, but they do not prove a
recommendation is ready. Before telling the manager "do this next", include at
least one realistic dry-run or real-use signal:

- a generated radar report from the selected repo;
- a projectless portable fallback run when portability matters;
- a bounded `-RefreshEcosystem` run when external ecosystem refresh is the
  recommendation; or
- a linked issue/PR/check/comment gate for blocked or unfinished work.

If the signal is fixture-only, static-only, list-only, or missing, label the
recommendation `UNVERIFIED` and make the next action evidence collection rather
than adoption.
...
8. Apply the recommendation ladder before final wording. The first item in
   `다음에 할 일` must be the highest-ranked evidence-backed action, not just the
   newest or most interesting repository.
...
If this optimization makes recommendations noisy or weakens evidence labels,
restore `examples\skillopt-medium-risk-rollout\manager-next-work-radar\rollback\SKILL.md`
over both Codex Desktop `manager-next-work-radar` skill targets, then rerun:

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\Test-SkillOptimizationValidationHarness.ps1 -SpecPath examples\skillopt-medium-risk-rollout\manager-next-work-radar\pilot.json -Json
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\Test-ManagerNextWorkRadar.ps1
```
