...
5. Treat unresolved review threads, requested changes, linked issue comment
   follow-up, unknown mergeability, and failed checks as blockers to readiness.
6. Update PR evidence or final report with what was fixed, what was tested, and
   what remains blocked.
7. If the PR is clean and merge was requested or routine auto-merge rules allow
...
- If GitHub data is unavailable, report `blocked/unverified` with the exact
  command failure and next retry condition instead of asking the manager to
  inspect raw review state.
...
Rollback:
- Restore the prior `pr-feedback-resolver` skill text or rollback artifact.
- Rerun the PR feedback harness spec and the current-PR mergeability/comment
  gate before claiming readiness again.
